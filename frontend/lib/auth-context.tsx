...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (error) { console.error("Google OAuth error:", error); return }
  ...
  const signInWithGithub = async () => {
    try {
      ...
      if (error) { console.error("GitHub OAuth error:", error); return }
      if (data?.url) { window.location.href = data.url; return }
      console.error("GitHub OAuth: no URL returned")
    } catch (err) {
